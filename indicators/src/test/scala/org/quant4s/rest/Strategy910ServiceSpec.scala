  ...
  "策略管理全过程, ID: 910，911，912" - {
    "  创建三个策略910, 911, 912" in {
      ...
      Get("/strategy/list")~> strategyServiceRoute ~> check {
        ...
        ret.data.getOrElse(new Array[Strategy](1)).length shouldEqual 3
      }
    }

    "  读取存在的策略910" in {
      Post("/strategy", HttpEntity(MediaTypes.`application/json`,
        """{"id": 915,"name": "实盘测试","runMode":1, "status": 1}"""
      )) ~> strategyServiceRoute ~> check {
        //status shouldEqual Success
        responseAs[String] shouldEqual """{"code":0}"""
      }
      Get("/strategy/915") ~> strategyServiceRoute ~> check {
        val json = responseAs[String]
        implicit val formats = DefaultFormats
        val jv = parse(json)
        val ret = jv.extract[RetStrategy]
        ret.code shouldEqual 0
        ret.data.get.portfolio.get.cash shouldEqual 100000.0
      }
    }
    ...
    "更新策略912" in {
      ...
      Get("/strategy/912") ~> strategyServiceRoute ~> check {
        ...
        val ret = jv.extract[RetStrategy]
        ret.code shouldEqual 0
        ret.data.get.id shouldEqual 912
        ret.data.get.portfolio.get.cash shouldEqual 90000
      }
    }
    ...
    "运行一个策略910" in {
      ...
      Get("/strategy/910") ~> strategyServiceRoute ~> check {
        ...
        val ret = jv.extract[RetStrategy]
        ret.code shouldEqual 0
        ret.data.get.id shouldEqual 910
        ret.data.get.status shouldEqual 1
      }
    }
  }
}
