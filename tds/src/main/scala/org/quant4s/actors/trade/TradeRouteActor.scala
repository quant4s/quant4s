...
import org.quant4s.actors.persistence.AccountPersistorActor
import org.quant4s.actors._
...
class TradeRouteActor extends Actor with ActorLogging {
  ...
  val restRef = context.actorSelection("/user/" + PATH_HTTP_SERVER_ACTOR)
  ...
  private def _traderCreated(trader: TradeAccount): Unit = {
    ...
    try {
      val c = Class.forName(clazz)
      //.newInstance().asInstanceOf[TBrokerage]
